...
import type { AppConfig, ApplyPatchCommand, ApprovalPolicy, ResponseInputItem } from "./types.js";
import type { ExecInput } from "./sandbox/interface.js";

import { exec, execApplyPatch, formatCommandForDisplay } from "./exec.js";
import { isLoggingEnabled, log } from "./log.js";
import { ReviewDecision } from "./review.js";
import { FullAutoErrorMode } from "./types.js";
...
// A simple function to check if a command can be auto-approved
// In a real implementation, this would be more sophisticated and check against a policy
function canAutoApprove(command: Array<string>, policy: ApprovalPolicy, writableRoots: Array<string>): {
  type: "auto-approve" | "ask-user" | "reject";
  ...
  if (command[0] === "apply_patch" && command.length === 2) {
    return {
      type: "ask-user",
      applyPatch: { patch: command[1] },
    };
  }
  
  // Auto-approve common safe commands
  const safeCommands = ["ls", "pwd", "echo", "cat"];
  if (safeCommands.includes(command[0])) {
    return {
      type: "auto-approve",
      runInSandbox: false,
      reason: `Safe command: ${command[0]}`,
      group: "Utility",
    };
  }
  
  // In full-auto mode, approve everything but run in sandbox
...
export async function handleExecCommand(
  ...
  switch (safety.type) {
    ...
    case "auto-approve": {
      runInSandbox = safety.runInSandbox ?? false;
      break;
    }
    case "reject": {
      return {
