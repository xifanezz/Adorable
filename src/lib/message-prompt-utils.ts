...
export function truncateFileToolCalls(messages: Message[]): Message[] {
        ...
        if (messageCopy.parts) {
            for (const part of messageCopy.parts) {
                ...
                if (toolName === "read_file" || toolName === "read_multiple_files") {
                    ...
                    if (toolInvocation.state === "result" && toolInvocation.result) {
                        if (typeof toolInvocation.result.content === "string") {
                            toolInvocation.result.content = "[File content truncated to save tokens]";
                        } else if (Array.isArray(toolInvocation.result.content)) {
                            toolInvocation.result.content = [
                                { type: "text", text: "[File content truncated to save tokens]" }
                            ];
                        }
                    }
                }
                // Handle write_file and edit_file tool calls
                    ...
                    if (toolInvocation.args && typeof toolInvocation.args === "object") {
                        if ("content" in toolInvocation.args) {
                            toolInvocation.args.content = "[Content truncated to save tokens]";
                        }
                        if ("edits" in toolInvocation.args && Array.isArray(toolInvocation.args.edits)) {
                    ...
                    if (toolInvocation.state === "result" && toolInvocation.result) {
                        if (typeof toolInvocation.result.content === "string" &&
                            ...
                            toolInvocation.result.content = "[Diff output truncated to save tokens]";
                        } else if (Array.isArray(toolInvocation.result.content)) {
                            toolInvocation.result.content = [
                                { type: "text", text: "[Diff output truncated to save tokens]" }
                            ];
                        }
                    }
                }
                // Handle directory listings - truncate large outputs
                    ...
                    if (toolInvocation.state === "result" && toolInvocation.result) {
                        if (Array.isArray(toolInvocation.result.content) &&
                            ...
                            const firstFewLines = originalText.split('\n').slice(0, 5).join('\n');
                            toolInvocation.result.content = [
                                {
                                    type: "text",
                ...
                // Handle exec tool - truncate long commands and outputs
                    ...
                    if (toolInvocation.args && typeof toolInvocation.args === "object" && "command" in toolInvocation.args) {
                        const command = String(toolInvocation.args.command);
                        if (command.length > 200) {
                            toolInvocation.args.command = command.substring(0, 100) + "... [command truncated to save tokens]";
                        }
                    }
                        ...
                        if (typeof toolInvocation.result.content === "string") {
                            ...
                            if (content.length > 500) {
                                ...
                                if (lines.length > 20) {
                                    ...
                                    const truncatedContent = [
                                        ...lines.slice(0, 10),
                                        "\n[... exec output truncated to save tokens (removed " + (lines.length - 15) + " lines) ...]\n",
                                        ...lines.slice(-5)
                                    ].join('\n');
                                    toolInvocation.result.content = truncatedContent;
                                } else if (content.length > 1000) {
                                    // If it's just a few very long lines
                                    toolInvocation.result.content = content.substring(0, 300) +
                            ...
                            if (text.length > 500) {
                                ...
                                if (lines.length > 20) {
                                    ...
                                    toolInvocation.result.content = [{ type: "text", text: truncatedText }];
    ...
    return [...truncatedMessages, ...messagesToKeep.map(m => structuredClone(m))];
}
